        ...
        switch (recordType.toLowerCase()) {
            case "amountofwater":
                ...
                AmountOfWaterRecord amountOfWaterRecord = amoWaterRepo.findById(record_id)
                        .orElseThrow(()->new ResourceNotFoundException("Water record with id " + record_id + "not exist"));
            ...
            case "moisture":
                ...
                MoistureRecord moistureRecord = moiRepo.findById(record_id)
                        .orElseThrow(()->new ResourceNotFoundException("Moisture record with id" + record_id + "not exist"));
                moiRepo.delete(moistureRecord);
                return  "deleted a Moisture Record.";
            case "light":
                LightRecordRepository lightRepo = recordRepositoryFactory.getRecordRepository(LightRecordRepository.class);
                LightRecord lightRecord = lightRepo.findById(record_id)
                        .orElseThrow(()->new ResourceNotFoundException("Light record with id" + record_id + "not exist"));
                lightRepo.delete(lightRecord);
                return  "deleted a Light Record.";
            case "humidity":
                HumidityRecordRepository humiRepo = recordRepositoryFactory.getRecordRepository((HumidityRecordRepository.class));
                HumidityRecord humidityRecord = humiRepo.findById(record_id)
                        .orElseThrow(()->new ResourceNotFoundException("Humidity record with id" + record_id + "not exist"));
            ...
            case "temperature":
                ...
                TemperatureRecord temperatureRecord = temRepo.findById(record_id)
                        .orElseThrow(()->new ResourceNotFoundException("Tempurature record with id" + record_id + "not exist"));
